...
# 4/28/2022 This tool is a modification of the NASISpedons_Extract_Pedons_from_NASIS _MultiThreading_ArcGISPro_SQL.py
# script.  It was modified\created for the purpose of mining all pedons from
# NASIS to create a complete pedon dataset (spatial and tabular) that can be copied over to the
# SMB04 drive.  This script is part of a scheduled task that gets executed daily.
...
# =========================================== Main Body ==========================================
# Import modules
import sys, string, os, traceback, re, arcpy, socket, time, urllib, multiprocessing, requests, shutil
...
if __name__ == '__main__':
    ...
    try:
        ...
        inputFeatures = ''
        DBname = f"NASIS_Pedons_ALL_{date.replace(' ','_')}_Interim"
        outputFolder = r'N:\flex\NCSS_Pedons\NASIS_Pedons\Web_Feature_Service'
        finalDB = f"{outputFolder}\\NASIS_Pedons_ALL_{date.replace(' ','_')}.gdb"
        sqliteFormat = False
        allPedons = True
        ...
        textFilePath = outputFolder + os.sep + "NASIS_Pedons_ALL_Weekly_ScheduledTask_LogFile.txt"
        startTime = tic()
...
##            totalPedons = len(pedonDict)
        ...
        # Proceed with deleting the last final version of ALL pedons and replacing it with the interim
        if totalPedons == pedonCount or ((totalPedons - pedonCount) / totalPedons) < totalPedons * 0.0005:
            arcpy.env.workspace=outputFolder

            # Delete old NASIS_Pedons_All* FGDB
            try:
                lastVersion = arcpy.ListWorkspaces('NASIS_Pedons_ALL*','FileGDB')

                if len(lastVersion) > 1:
                    for gdbVersion in lastVersion:
                        if not gdbVersion == pedonDB:
                            arcpy.Delete_management(gdbVersion)
                            AddMsgAndPrint(f".\nSuccessfully deleted {os.path.basename(gdbVersion)} FGDB")
                elif not lastVersion[0] == pedonDB:
                    arcpy.Delete_management(lastVersion)
                    AddMsgAndPrint(f".\nSuccessfully deleted {os.path.basename(lastVersion)} FGDB")
                else:
                    pass
            except:
                pass

            arcpy.Rename_management(pedonDB,finalDB[:-4])
            smb04drive = r'\\aiomokcc6na102\SMB04\Soil_Plant_Science_Division\NSSC\Pedons\NASIS_Pedons'
            finalSMBpath = f"{smb04drive}\{os.path.basename(finalDB)}"

            # Remove the FGDB in the SMB04 path
            if os.path.exists(finalSMBpath):
                shutil.rmtree(finalSMBpath)

            shutil.copytree(finalDB,finalSMBpath)
            AddMsgAndPrint(".\nSuccessfully copied {os.path.basename(pedonDB)} to SMB04 drive")
