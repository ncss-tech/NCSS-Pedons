...
## ===================================================================================
def addPedonReports(pedonFC):

    try:

        # New field that will be added to PedonFC
        fieldName = "pedondescriptionreport"
        arcpy.AddField_management(pedonFC, fieldName,'TEXT',field_length=155, field_alias="Pedon Description Report", field_is_nullable="NULLABLE")
...
##        allPedons = False
        ...
        textFilePath = outputFolder + os.sep + GDBname + "_logFile.txt"
        startTime = tic()
...
##                importPedonDataByTable(table)
        ...
        if not addPedonReportHyperlink(pedonFGDBfc):
        ...
        try:
            if arcpy.GetCount_management(pedonFGDBfc).inputCount > 0:
                aprx = arcpy.mp.ArcGISProject("CURRENT")
                aprxMap = aprx.listMaps()[0]
                aprxMap.addDataFromPath(pedonFGDBfc)
