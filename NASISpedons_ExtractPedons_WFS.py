...
##        allPedons = arcpy.GetParameter(3)
        ...
        DBname = r'NASIS_Pedons_WFS_Interim'
        outputFolder = r'E:\GIS_Projects\Task_Scheduler_Test'
        sqliteFormat = False
        allPedons = True
        ...
        # Text file path - Start the timer
...
##        now = time.localtime()
##        date = f"{now.tm_mon}/{now.tm_mday}/{now.tm_year}"
##        time = f"{now.tm_hour}:{now.tm_min}:{now.tm_sec}"
        now = datetime.datetime.today()
        date = now.strftime("%B %I %Y")
        ...
        try:
            ...
            m.write(f"{os.linesep}{date},{hour},{FinishTime},{pedonCount}")
            m.close
            del m
        except:
            errorMsg()
